<template>
  <div class="edid clearfix center">
  <h1>编辑用户</h1>
    <div class="form_box"><form v-on:submit="editForm">
      <div class="form_cen">
        <h2>用户信息</h2>
        <div class="form_p">
          <p>姓名</p>
...
export default {
  ...
  methods:{
    ...
    editForm(e){
     if(!this.customer.name || !this.customer.phone  || !this.customer.email){
       ...
       this.$http.put("http://localhost:3000/users/"+this.$route.params.id,editdata)
         .then(function(data){
         //提交成功后返回到主页
           this.$router.push({path:"/",query:{alert:"用户更新信息成功"}})   //path：路径，query：传递的信息
         })
     }
  ...
  created:function(){
    this.databys(this.$route.params.id)
  }
}
</script>
